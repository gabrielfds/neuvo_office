...
function buildConfig() {
  ...
  // Layer 2: Auto-detect gateway token from OpenClaw config if not set
  if (!config.gateway.token || config.gateway.token === 'your-gateway-token-here') {
    try {
      ...
      if (existsSync(openclawConfigPath)) {
        ...
        const autoToken = ocConfig?.gateway?.auth?.token
        if (autoToken) {
          config.gateway.token = autoToken
          console.log('[config] Auto-detected gateway token from ~/.openclaw/openclaw.json')
        }
      }
    } catch {}
  }
